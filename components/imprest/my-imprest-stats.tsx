  ...
  const stats = useMemo(() => {
    ...
    try {
      ...
      imprests.forEach((imprest: MyImprest) => {
        ...
        if (status === "pending_accountant" || status === "pending_hod" || status === "disbursed") {
          activeAmount += amount
        }
        ...
        if (
          ...
          try {
            ...
            const processingTime = (approvalDate.getTime() - requestDate.getTime()) / (1000 * 60 * 60) // hours
        ...
        if (imprest.accounting && imprest.accounting.verifiedAt) {
          recentActivity.push({
            ...
            details: `You accounted for ${formatCurrency(amount, currency)} (${imprest.paymentReason})`,
        ...
        if (imprest.disbursement && imprest.disbursement.disbursedAt) {
          recentActivity.push({
            ...
            details: `${formatCurrency(amount, currency)} disbursed for ${imprest.paymentReason}`,
            imprestId: imprest._id,
          })
        }
      ...
      return {
        ...
        totalAmount,
        activeImprests:
          (statusCounts.pending_accountant || 0) + (statusCounts.pending_hod || 0) + (statusCounts.disbursed || 0),
        activeAmount,
        accountedImprests: statusCounts.accounted || 0,
        pendingImprests: (statusCounts.pending_accountant || 0) + (statusCounts.pending_hod || 0),
    ...
    return (
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-6 text-center dark:border-slate-800 dark:bg-slate-900/50">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          You haven&apos;t made any imprest requests yet. Click the button below to create your first request.
        </p>
      ...
      <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="overflow-hidden border-none shadow-lg bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 hover:shadow-xl transition-shadow duration-300">
          <CardHeader className="pb-2 bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-950/50 dark:to-blue-900/50">
            ...
            <CardDescription>Your imprest requests summary</CardDescription>
          </CardHeader>
            ...
            <div className="space-y-2.5">
              <div>
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-bold">{stats.totalImprests || 0}</span>
                </div>
                <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                  Total value: {formatCurrency(stats.totalAmount || 0, defaultCurrency)}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex h-2.5 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
                  <div
                    ...
                    style={{
                      width: `${
                        stats.totalImprests > 0 ? ((stats.accountedImprests || 0) / stats.totalImprests) * 100 : 0
                      }%`,
                    }}
                  />
                  <div
                    ...
                    style={{
                      width: `${
                        stats.totalImprests > 0 ? ((stats.pendingImprests || 0) / stats.totalImprests) * 100 : 0
                      }%`,
                    }}
                  />
                  <div
                    ...
                    style={{
                      width: `${
                        stats.totalImprests > 0 ? ((stats.accountingRequired || 0) / stats.totalImprests) * 100 : 0
                      }%`,
                    }}
                  />
                  <div
                    ...
                    style={{
                      width: `${
                        stats.totalImprests > 0 ? ((stats.rejectedImprests || 0) / stats.totalImprests) * 100 : 0
                      }%`,
                    }}
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 dark:border-slate-800 dark:bg-slate-900/50">
                  <div className="text-xs font-medium text-slate-500 dark:text-slate-400">Active Imprests</div>
                  <div className="mt-1 text-lg font-semibold">{stats.activeImprests || 0}</div>
                 
                </div>
                <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 dark:border-slate-800 dark:bg-slate-900/50">
                  <div className="text-xs font-medium text-slate-500 dark:text-slate-400">Avg. Processing</div>
        ...
        <Card className="overflow-hidden border-none shadow-lg bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 hover:shadow-xl transition-shadow duration-300">
          <CardHeader className="pb-2 bg-gradient-to-r from-amber-50 to-amber-100 dark:from-amber-950/50 dark:to-amber-900/50">
            ...
            <CardDescription>Your ongoing imprest requests</CardDescription>
          </CardHeader>
          <CardContent className="pt-4">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm font-medium text-slate-500 dark:text-slate-400">Active Amount</div>
                  <div className="text-2xl font-bold">{formatCurrency(stats.activeAmount || 0, defaultCurrency)}</div>
                </div>
                <div>
                  <div className="text-sm font-medium text-slate-500 dark:text-slate-400">Requests</div>
                ...
                <div className="space-y-2 max-h-[140px] overflow-y-auto pr-1">
                  {stats.upcomingDeadlines && stats.upcomingDeadlines.length > 0 ? (
                    stats.upcomingDeadlines.map((imprest, i) => (
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <div>
                            <p className="text-xs font-medium line-clamp-1">{imprest.paymentReason}</p>
                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                              {formatCurrency(imprest.amount || 0, imprest.currency || defaultCurrency)}
                            </p>
                          </div>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Badge variant="outline" className={getStatusBadgeVariant(imprest.status)}>
                            ...
                            <TooltipContent>
                              ...
                              {imprest.dueDate && (
                                <p>
                                  ...
                                  <span className={getDaysRemainingColor(calculateDaysRemaining(imprest.dueDate))}>
                                    {calculateDaysRemaining(imprest.dueDate)} days
                                  </span>
                                  )
                                </p>
                              )}
                    ...
                    <div className="text-center py-4 rounded-lg border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-900/50">
                      <p className="text-sm text-slate-500 dark:text-slate-400">No active imprests</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 dark:border-slate-800 dark:bg-slate-900/50">
                <div className="flex items-center justify-between">
                  <div className="text-xs font-medium text-slate-500 dark:text-slate-400">Monthly Trend</div>
                ...
                <div className="mt-2 flex items-end justify-between h-10 gap-1">
                  {monthlyTrendSorted.length > 0 ? (
                    monthlyTrendSorted.map(([month, amount], i) => {
                      const maxAmount = Math.max(...monthlyTrendSorted.map(([_, val]) => val))
                      const height = maxAmount > 0 ? (amount / maxAmount) * 100 : 0
                      return (
                        <Tooltip key={i}>
                          <TooltipTrigger asChild>
                            <div
                              className="bg-amber-500 dark:bg-amber-600 rounded-t w-full hover:bg-amber-600 dark:hover:bg-amber-500 transition-colors"
                              style={{ height: `${Math.max(height, 10)}%` }}
                            ></div>
                          </TooltipTrigger>
                          <TooltipContent>
                            <p>
                              {month}: {formatCurrency(amount, defaultCurrency)}
                            </p>
                          </TooltipContent>
                        </Tooltip>
                      )
                    ...
                    <div className="w-full text-center text-xs text-slate-400">No data available</div>
                  )}
                </div>
              </div>
            </div>
        ...
        <Card className="overflow-hidden border-none shadow-lg bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 hover:shadow-xl transition-shadow duration-300">
          <CardHeader className="pb-2 bg-gradient-to-r from-emerald-50 to-emerald-100 dark:from-emerald-950/50 dark:to-emerald-900/50">
            ...
            <CardDescription>Imprest accounting requirements</CardDescription>
          </CardHeader>
          <CardContent className="pt-4">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm font-medium text-slate-500 dark:text-slate-400">Accounting Required</div>
                  <div className="text-2xl font-bold">{stats.accountingRequired || 0}</div>
                </div>
                <div>
                  <div className="text-sm font-medium text-slate-500 dark:text-slate-400">Amount</div>
                  <div className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">
                    {formatCurrency(stats.accountingRequiredAmount || 0, defaultCurrency)}
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-2 max-h-[140px] overflow-y-auto pr-1">
                  {stats.upcomingDeadlines && stats.upcomingDeadlines.length > 0 ? (
                          ...
                          >
                            <div className="flex items-start justify-between">
                              ...
                              <Tooltip>
                                ...
                                <TooltipContent>
                                  <p>
                                    {formatCurrency(imprest.amount || 0, imprest.currency || defaultCurrency)} disbursed
                                    on {imprest.disbursement ? formatDate(imprest.disbursement.disbursedAt) : "N/A"}
                                  </p>
                                </TooltipContent>
                              </Tooltip>
                            </div>
                    ...
                    <div className="text-center py-4 rounded-lg border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-900/50">
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400">No accounting required</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 dark:border-slate-800 dark:bg-slate-900/50">
                <div className="text-xs font-medium text-slate-500 dark:text-slate-400">Accounting Completion</div>
                <div className="mt-2">
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span>Completed</span>
                    <span>
                      {stats.accountedImprests || 0}/{stats.totalImprests || 0} (
                      {formatPercentage(
                        stats.totalImprests > 0 ? ((stats.accountedImprests || 0) / stats.totalImprests) * 100 : 0,
                      )}
                      )
                  ...
                  <Progress
                    value={stats.totalImprests > 0 ? ((stats.accountedImprests || 0) / stats.totalImprests) * 100 : 0}
                    className="h-1.5"
                  />
                </div>
              </div>
        ...
        <Card className="overflow-hidden border-none shadow-lg bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 hover:shadow-xl transition-shadow duration-300">
          <CardHeader className="pb-2 bg-gradient-to-r from-violet-50 to-violet-100 dark:from-violet-950/50 dark:to-violet-900/50">
            ...
            <CardDescription>Latest updates on your imprests</CardDescription>
          </CardHeader>
          <CardContent className="pt-4">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-2xl font-bold">{stats.accountedImprests || 0}</div>
                </div>
                <div>
                  <div className="text-sm font-medium text-slate-500 dark:text-slate-400">Success Rate</div>
                  <div className="text-2xl font-bold text-violet-600 dark:text-violet-400">
                    {formatPercentage(
                      stats.totalImprests > 0
                        ? ((stats.accountedImprests || 0) / (stats.totalImprests - (stats.pendingImprests || 0))) * 100
                        : 0,
                    )}
                  </div>
                </div>
                      ...
                      >
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-xs line-clamp-2">{activity.details || "Activity details unavailable"}</p>
