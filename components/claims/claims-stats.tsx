...
import { Clock, FileCheck2, CheckCircle2, Users, Building2, Hourglass, Calendar } from "lucide-react"
...
const groupStatusesForDisplay = (statusDistribution: [string, number][]): [string, number][] => {
...
export default function ClaimsStats({claimsData }: { claimsData: any }) {
    ...
    const calculateStats = async () => {
      try {
        ...
        claimsData.forEach((claim: Claim) => {
          ...
          if (claim.status === "approved" && claim.auditTrail && claim.auditTrail.length > 0 && claim.createdAt) {
            try {
              const createdDate = new Date(claim.createdAt)
              const lastApprovalEntry = claim.auditTrail[claim.auditTrail.length - 1]

              if (lastApprovalEntry && lastApprovalEntry.performedAt) {
                const approvalDate = new Date(lastApprovalEntry.performedAt)

                if (!isNaN(createdDate.getTime()) && !isNaN(approvalDate.getTime())) {
                  const approvalTimeHours = (approvalDate.getTime() - createdDate.getTime()) / (1000 * 60 * 60)
          ...
          if (claim.status && claim.status.includes("pending") && claim.createdAt) {
            try {
              const createdDate = new Date(claim.createdAt)
        ...
        const approvalRatio = totalClaims > 0 ? (statusDistribution["approved"] || 0) / totalClaims : 0
        const pendingApprovals = Object.entries(statusDistribution)
          .filter(([status]) => status.includes("pending"))
          .reduce((sum, [, count]) => sum + count, 0)
        const averagePercentageClaimed = totalMilestones > 0 ? totalPercentageClaimed / totalMilestones : 0
  ...
  const departmentDistribution = useMemo(() => {
    return Object.entries(stats.departmentDistribution).sort(([, a], [, b]) => b - a)
              ...
              <div className="space-y-4">
                  ...
                  <div className="relative h-16 w-16">
                    <svg className="h-16 w-16 rotate-[-90deg]" viewBox="0 0 100 100">
                      ...
                      <circle
                        ...
                        strokeDashoffset={
                          stats.totalClaims > 0 ? 251.2 - 251.2 * (stats.pendingApprovals / stats.totalClaims) : 251.2
                        }
                        strokeLinecap="round"
                      />
                    </svg>
                    <div className="absolute inset-0 flex items-center justify-center text-sm font-medium">
                      {stats.totalClaims > 0 ? Math.round((stats.pendingApprovals / stats.totalClaims) * 100) : 0}%
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex h-3 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
                    {groupedStatusDistribution.map(([status, count], i) => {
                      let bgColor = "bg-slate-500"
                      if (status === "Approved") bgColor = "bg-emerald-500"
                      if (status === "Rejected") bgColor = "bg-rose-500"
                      if (status === "Pending") bgColor = "bg-amber-500"
                      ...
                      return (
                        <div
                          ...
                          style={{
                            width: `${stats.totalClaims > 0 ? (count / stats.totalClaims) * 100 : 0}%`,
                          }}
                        />
                      )
                    })}
                  ...
                  <div className="flex justify-between mt-2 text-xs text-muted-foreground">
                    {groupedStatusDistribution.map(([status, count], i) => (
                      <div key={i} className="flex items-center gap-1">
                        ...
                        ></span>
                        <span>
                          {status}: {Math.round((count / stats.totalClaims) * 100)}%
                        </span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {statusDistribution.slice(0, 6).map(([status, count], i) => (
                      <Tooltip key={i}>
                        <TooltipTrigger asChild>
                          <Badge
                            variant="outline"
                            className={`${getStatusBadgeVariant(status as ClaimStatus)} hover:bg-opacity-80 transition-colors`}
                          >
                            {getReadableStatus(status as ClaimStatus)}: {count}
                          </Badge>
                        </TooltipTrigger>
                        <TooltipContent>
                          <p>{Math.round((count / stats.totalClaims) * 100)}% of total claims</p>
                ...
                <div className="pt-3">
                  <p className="text-sm font-medium mb-1.5">Approval Time</p>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Average</span>
                    <span className="font-medium">{formatDuration(stats.averageApprovalTime)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Fastest</span>
                    <span className="font-medium text-emerald-600">{formatDuration(stats.fastestApproval)}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-3 gap-4">
                  {departmentDistribution.slice(0, 3).map(([department, count], i) => (
                    <div
                      key={i}
                    ...
                    >
                      <div className={`h-3 w-3 rounded-full ${i === 0 ? "bg-violet-500" : "bg-amber-500"} mb-1`}></div>
                      <p className="text-xl font-bold">{count}</p>
                      <p className="text-xs text-muted-foreground text-center">{department}</p>
                    </div>
                  ))}
                </div>
                  ...
                  {departmentDistribution.map(([department, count], i) => (
                    <div key={i} className="mb-2">
                      <div className="flex items-center justify-between text-sm mb-1">
                        ...
                        <span className="font-medium">{formatCurrency(stats.departmentAmounts[department] || 0)}</span>
                      </div>
                      <div className="flex h-2 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
                        <div
                          className={i % 2 === 0 ? "bg-violet-500" : "bg-amber-500"}
                          style={{
                            width: `${stats.totalClaims > 0 ? (count / stats.totalClaims) * 100 : 0}%`,
                          }}
                        />
                      ...
                      <div className="flex justify-end mt-1">
                        <span className="text-xs text-muted-foreground">
                          {Math.round((count / stats.totalClaims) * 100)}% of claims
                        </span>
                      </div>
                    </div>
                  ))}
                ...
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p>
                      ...
                      {Math.round((departmentDistribution[0][1] / stats.totalClaims) * 100)}%)
                    </p>
                    <p>{formatCurrency(stats.departmentAmounts[departmentDistribution[0][0]] || 0)} total value</p>
                  </TooltipContent>
                </Tooltip>
              )}
            </div>
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="space-y-4">
                ...
                <div>
                  <p className="text-sm font-medium mb-2">Claims by Age</p>
                ...
                <div className="pt-3">
                  <p className="text-sm font-medium mb-1.5">Milestone Progress</p>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-sm text-muted-foreground">Average Claimed</span>
                    <span className="font-medium">{Math.round(stats.averagePercentageClaimed)}%</span>
                  </div>
                  <Progress
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Badge
                    ...
                    className="bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100 transition-colors"
                  >
                    {stats.recentActivity.lastApproved ? getTimeAgo(stats.recentActivity.lastApproved) : "N/A"}
                  </Badge>
                </TooltipTrigger>
                <TooltipContent>
                  {stats.recentActivity.lastApproved ? (
                    <>
                      ...
                      <p className="font-medium">{formatDate(stats.recentActivity.lastApproved)}</p>
                    </>
                  ) : (
