...
import { ArrowUpDown, ChevronDown, Eye, Filter, Search, FileDown } from "lucide-react"
  ...
  const columns: ColumnDef<Claim>[] = [
    {
      ...
      cell: ({ row }) => <div className="font-medium text-blue-700">{row.original.contractId?.contractNumber}</div>,
    },
    {
      accessorKey: "projectId.name",
      header: "Project Name",
      cell: ({ row }) => <div className="font-medium">{row.original.projectId?.name}</div>,
    },
    {
      accessorKey: "amount",
      header: ({ column }) => {
      ...
      cell: ({ row }) => {
        return <div className="font-medium">{formatCurrency(row.original?.amount, row.original?.currency)}</div>
      },
    },
    {
      accessorKey: "status",
      ...
      cell: ({ row }) => {
        ...
        return (
          <Badge
            variant={status === "approved" ? "success" : status === "rejected" ? "destructive" : status.includes("pending") ? "warning" : "warning"}
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="space-y-4">
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            View and manage all your submitted claims. Click on a claim to see more details.
          </CardDescription>
        </CardHeader>
